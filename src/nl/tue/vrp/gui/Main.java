...
public class Main {
    ...
    private static GUIConfig loadConfig() {
        Yaml yaml = new Yaml(new Constructor(GUIConfig.class));
        InputStream inputStream = Main.class
                .getClassLoader()
                .getResourceAsStream("config.yaml");
        if (inputStream == null) throw new IllegalArgumentException("config.yaml is not found");
...
//        JPanel headerPanel = getPanel(Color.RED);
        ...
        submitButton.addActionListener(e -> {
            ...
            for (Map.Entry<String, CodeInput> entry : codeInputMap.entrySet()) {
                ...
                Object obj = CClassLoader.loadFromFile(code, name + ".java", entry.getValue().getSelectedAlgoName());
                if (entry.getValue().getSelectedAlgo() == null) throw new RuntimeException("NULL object " + entry.getKey());
