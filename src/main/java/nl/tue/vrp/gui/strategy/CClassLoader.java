...
//        sb.append("}\n");
        ...
        File classJava = new File("tmp/%s".formatted(filename));
        if (classJava.getParentFile().exists() || classJava.getParentFile().mkdirs()) {
            try {
                try (Writer writer = new FileWriter(classJava)) {
                    writer.write(code);
                    writer.flush();
                }
...
//                System.out.println(System.getProperty("jdk.module.path"));
                ...
                optionList.add("%s%s%s".formatted(System.getProperty("java.class.path"), File.pathSeparator, System.getProperty("jdk.module.path")));

                Iterable<? extends JavaFileObject> compilationUnit
                        = fileManager.getJavaFileObjectsFromFiles(List.of(classJava));
